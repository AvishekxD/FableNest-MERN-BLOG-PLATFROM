...
const FeaturedPosts = () => {
  ...
  if (!posts || posts.length === 0) {
    // Optionally render a message or null if no posts are found
    return null; // or <div>No featured posts available.</div>;
  }

  // Ensure we have enough posts before trying to access them
  // This prevents runtime errors if the API returns fewer than 4 posts
  if (posts.length < 4) {
    // You might want to handle this case more gracefully,
    // e.g., by rendering a partial layout or different content.
    // For simplicity, we'll just return null if not enough posts.
    console.warn("Not enough featured posts returned for the layout.");
    return null;
  }
