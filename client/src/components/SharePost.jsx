...
const SharePost = ({ title, slug }) => {
  ...
  const links = generateShareLinks({ title, slug });
  ...
  const TwitterIcon = () => (
    <svg className="w-5 h-5 fill-current text-blue-400 hover:text-blue-500" viewBox="0 0 24 24">
      <path d="M23 3a10.9 10.9 0 01-3.14 1.53 4.48 4.48 0 00-7.86 3v1A10.66 10.66 0 013 4s-4 9 5 13a11.64 11.64 0 01-7 2c9 5 20 0 20-11.5a4.5 4.5 0 00-.08-.83A7.72 7.72 0 0023 3z" />
    </svg>
  );

  const LinkedInIcon = () => (
    <svg className="w-5 h-5 fill-current text-blue-600 hover:text-blue-700" viewBox="0 0 24 24">
  ...
  const EmailIcon = () => (
    <svg className="w-5 h-5 fill-current hover:text-red-500" viewBox="0 0 22 22">
      <path d="M12 13.5l8-6v-1.5l-8 6-8-6v1.5l8 6zM4 6h16v12h-16v-12z" />
    </svg>
  );

  const CopyIcon = () => (
    <svg className="w-5 h-5 text-white hover:text-[var(--Accent4)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  return (
    <div className="flex gap-3 mt-6 flex-wrap text-white flex-col">
      ...
      <div className="flex flex-row gap-1.5">
        <IconWrapper label="Twitter">
          <a href={links.twitter} target="_blank" rel="noopener noreferrer">
            <TwitterIcon />
          </a>
        </IconWrapper>

        <IconWrapper label="LinkedIn">
          <a href={links.linkedin} target="_blank" rel="noopener noreferrer">
            <LinkedInIcon />
          </a>
        </IconWrapper>

        <IconWrapper label="Gmail">
